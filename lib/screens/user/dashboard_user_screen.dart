            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                if (roomPvd.loading)
                  ...
                  Row(children: [
                    Expanded(child: StatCard(icon: Icons.bolt, iconColor: AppTheme.elecColor, label: 'Chỉ số điện', value: '${latestElec.toStringAsFixed(1)} kWh', trend: _calcTrend(billPvd.bills, isElec: true))),
                    const SizedBox(width: 12),
                    Expanded(child: StatCard(icon: Icons.water_drop, iconColor: AppTheme.waterColor, label: 'Chỉ số nước', value: '${latestWater.toStringAsFixed(1)} m³', trend: _calcTrend(billPvd.bills, isElec: false))),
                  ]),
                  const SizedBox(height: 16),
                ],
                // Hóa đơn cần xử lý
